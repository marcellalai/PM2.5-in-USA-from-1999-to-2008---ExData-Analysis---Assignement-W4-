...
plot5 <- function()
{
  ...
  totEmiss_vehicule_Balth <- aggregate(Emissions ~ year , NEI_Data_vehicule_Balth , FUN = sum )
  
  
  # plotting graph5 : emissions/year in Baltimore for motor vehicles
  plottingBar <- ggplot(data =  totEmiss_vehicule_Balth, aes(year, Emissions)) 
  plottingBar <- plottingBar + geom_bar(stat="identity")  + geom_label(label=round(totEmiss_vehicule_Balth$Emissions,1) , col = 1:4) +
           labs( x = "year", y = (expression("PM2.5 Emissions")) , title = " Total PM2.5 Annual Vehicle Emissions in Baltimore (MD) - 1999/2008" ) +
