...
plot4 <- function()
{
  ...
  # Filtring EI.Sector column  for words containing "coal" and subsetting NEI dataset with contribution from coal combustion
  SCC_Data_coal <- SCC_Data[grep("[Cc][Oo][Aa][Ll]",SCC_Data$EI.Sector),]
  NEI_Data_coal <- subset(NEI_Data, NEI_Data$SCC %in%  SCC_Data_coal$SCC )
  
  totEmiss_Coal <- aggregate(Emissions ~ year , NEI_Data_coal , FUN = sum )
  
  # plotting graph4 : emissions/year 
   
  with (totEmiss_Coal, barplot(height = Emissions, width = 1, space = NULL, names.arg = year, col = 15:18,
                          xlab = "year", ylab = "PM2.5 Annual Emission" , 
                          main = expression("Total PM2.5 Annual Coal Combustion Emissions in USA - 1999/2008")))
